...
CString  GetGongWeiName(CString& FalgStr,int nSide)
{
	CString strGongWeiName;

	if (nSide==0)
	{
		strGongWeiName="相机0反面";
	}else
	{
		strGongWeiName="相机1正面";
	}
	return strGongWeiName;
}

void DeleteOldCache(CString strDir,int nSaveDays)
{
	CFileFind ff;
	CString strCacheDir;
	strCacheDir.Format("%s*.*",strDir);
	BOOL res = ff.FindFile(strCacheDir);
	while(res)
	{
		res = ff.FindNextFile();
		if (ff.IsDots())//剔除".0”和".."两个
		{
			continue;
		}
		...
		if(GetFileTime(hDir,&lpCreateTime,&lpAccessTime,&lpWriteTime))
		{
			...
			CTime tFileTm = CTime((int)screatetime.wYear,(int)screatetime.wMonth,(int)screatetime.wDay,(int)screatetime.wHour,(int)screatetime.wMinute,(int)screatetime.wSecond);
			CTime tTm =	CTime::GetCurrentTime();
			int nDayDiff = CTimeSpan(tTm-tFileTm).GetDays();
			if (nDayDiff>nSaveDays)
			{
				bDelete = TRUE;
			}
		}
...
void DeleteOldFileFolder(CString& strPath,int nSaveDays)
{
	...
	while(bFound) 
	{
		...
		{
			CString strNextPath,strName0;
			strName0=ff.GetFileName();
			strNextPath.Format("%s\\%s",strPath,strName0);
			bFound1   =   ffNext.FindFile(strNextPath   +   "\\*.* "); 

			while(bFound1)
			{
				bFound1   =   ffNext.FindNextFile(); 
				if(!ffNext.IsDots()&&ffNext.IsDirectory() ) 
				{
					/*	strName=ffNext.GetFileName();*/
					strFile=ffNext.GetFilePath();
...
void TileImages (HObject ho_Images, HObject *ho_TiledImage,HTuple TileOrder)
{
	...
	if ("vertical"==TileOrder)
	{
		...
		}catch(HException e)
		{
			...
			strImagePath.Format("%s\\Log\\ho_Images",GetCurrentAppPath());
			WriteObject(ho_Images,(HTuple)strImagePath);
			strTuplePath.Format("%s\\Log\\hv_OffsetRow.tup",GetCurrentAppPath());
			WriteTuple(hv_OffsetRow,(HTuple)strTuplePath);

			strTuplePath.Format("%s\\Log\\hv_OffsetCol.tup",GetCurrentAppPath());
			WriteTuple(hv_OffsetCol,(HTuple)strTuplePath);

			strTuplePath.Format("%s\\Log\\hv_OffsetDefault.tup",GetCurrentAppPath());
			WriteTuple(hv_OffsetDefault,(HTuple)strTuplePath);

		}

	}
	if ("horizontal"==TileOrder)
	{
		CountObj(ho_Images, &hv_Number);
		GetImageSize(ho_Images, &hv_Width, &hv_Height);
		TupleSum(hv_Width, &hv_WholeHeight);
		//
		hv_OffsetCol = 0;
		hv_SingleWidthConcat = HTuple();
		{
			...
			for (hv_i=1; hv_i.Continue(end_val6, step_val6); hv_i += step_val6)
			{
				SelectObj(ho_Images, &ho_ObjectSelected, hv_i);
				GetImageSize(ho_ObjectSelected, &hv_SingleWidth, &hv_SingleHeight);
				hv_SingleWidthConcat = hv_SingleWidthConcat.TupleConcat(hv_SingleWidth);
				TupleSum(hv_SingleWidthConcat, &hv_Sum);
				hv_OffsetCol = hv_OffsetCol.TupleConcat(hv_Sum);
			}
		}
		//
		TupleGenConst(hv_Number, 0, &hv_OffsetRow);
		TupleGenConst(hv_Number, -1, &hv_OffsetDefault);
		//
		try
		{
			TileImagesOffset(ho_Images, &(*ho_TiledImage), hv_OffsetRow, hv_OffsetCol, hv_OffsetDefault, 
				hv_OffsetDefault, hv_OffsetDefault, hv_OffsetDefault,  hv_WholeHeight, 
				HTuple(hv_Height[0]));
		}catch(HException e)
		{
...
DHInterface* LoadDHInterFaceDll(CString strDllName)
{
	...
	{
		CString strMessage;
		strMessage.Format("加载%s.dll异常",strDllName);
...
{
	UINT nReturn = GetPrivateProfileInt(lpAppName,lpKeyName,Value,lpFileName);
	return (int)nReturn;
}
