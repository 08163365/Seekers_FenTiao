...
{   
 ...
 // m_nShiftPixels     = 4 ;   
  ...
  m_crPointOK    = RGB(0, 255, 0) ;  // see also SetPlotColor 
  m_crPointNG    = RGB(255, 0, 0) ;  // see also SetPlotColor 
...
{   
  ...
  m_dOrginalLowerLimit=0.25*(3*dLower+dUpper);//此处是利用方程推出来的 2017.09.01
  m_dOrginalUpperLimit=0.25*(3*dUpper+dLower);
...
/////////////////////////////////////////////////////////////////////////////   
void COScopeCtrl::SetPointColor(COLORREF colorOK, COLORREF colorNG)
{
	m_crPointOK = colorOK;
	m_crPointNG = colorNG;
...
{   
  ...
  {   
	  ...
	  m_dcGrid.SetPixel (i, nTopGridPix, RGB(255,0,0)) ;   
	  m_dcGrid.SetPixel (i, nMidGridPix,    m_crGridColor) ;   
	  m_dcGrid.SetPixel (i, nBottomGridPix, RGB(255,0,0)) ;
  ...
  strTemp.Format ("%.*lf", m_nYDecimals, m_dOrginalUpperLimit) ;   
  m_dcGrid.TextOut (m_rectPlot.left-4, m_rectPlot.top+m_rectPlot.Height()/4, strTemp) ;   //此处为了显示客户要求的上下限2017.09.01
   
  // y min   
  m_dcGrid.SetTextAlign (TA_RIGHT|TA_BASELINE) ;   
  strTemp.Format ("%.*lf", m_nYDecimals, m_dOrginalLowerLimit) ;   
  m_dcGrid.TextOut (m_rectPlot.left-4, m_rectPlot.bottom-m_rectPlot.Height()/4, strTemp) ;   
...
/////////////////////////////////////////////////////////////////////////////   
double COScopeCtrl::AppendPoint(double dNewPoint,BOOL bPointOK)   
{   
...
if (m_bFirstDraw != TRUE)
	...
	if (TRUE == m_bPointOK)
	{
		br.CreateSolidBrush(m_crPointOK);
		pen.CreatePen(0,1,m_crPointOK);
		CBrush* pOldBr = m_dcPlot.SelectObject(&br);
		CPen* pPen = m_dcPlot.SelectObject(&pen);
		//m_dcPlot.Ellipse(prevX-3,prevY-3,prevX+3,prevY+3);
		m_dcPlot.Ellipse(currX-3,currY-3,currX+3,currY+3);
		m_dcPlot.SelectObject(pOldBr);
		m_dcPlot.SelectObject(pPen);
	}else if (FALSE == m_bPointOK)
	{
		br.CreateSolidBrush(m_crPointNG);
		pen.CreatePen(0,1,m_crPointNG);
