...
void CXmvImageWnd::DispRegion()
{
	for (int i=0;i<m_hvRegionColor.Length();i++)
	{
		...
		DispObj(m_vec_hoCurrentDispRegion[i],m_hDispWnd);	
	}
}
...
BOOL CXmvImageWnd::Create(LPCTSTR lpszText, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	...
	SetCheck("~father");
	SetWindowAttr("background_color","dark olive green");
...
BOOL CXmvImageWnd::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	...
	case IDR_ZOOM_P:
		...
		break;
	case IDR_ZOOM_IN:
		hcur = LoadCursor(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDC_CURSOR_ZOOMIN));
		break;
	case IDR_ZOOM_OUT:
		hcur = LoadCursor(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDC_CURSOR_ZOOMOUT));
		break;
	case IDR_ZOOM_MOVE:
