...
CLineToolPara CLineToolPara::AffineTrans(const HTuple& HomMat2d,int& nLineIndex)
{
	...
	tNewLine.m_nSortType     = m_nSortType;

	if (nLineIndex==0 || nLineIndex==1 || nLineIndex==2 ||nLineIndex==3)
	{
	...
	{
		HTuple hvOriginalMat2D;
		HomMat2dIdentity(&hvOriginalMat2D);
		HTuple RowTrans,ColTrans;
		AffineTransPixel(hvOriginalMat2D,m_dStartRow,m_dStartCol,&RowTrans,&ColTrans);
		tNewLine.m_dStartRow = RowTrans.D();
		tNewLine.m_dStartCol = ColTrans.D();
		AffineTransPixel(hvOriginalMat2D,m_dEndRow,m_dEndCol,&RowTrans,&ColTrans);
		tNewLine.m_dEndRow = RowTrans.D();
		tNewLine.m_dEndCol = ColTrans.D();
	}
...
int CLineToolPara::ReadParam(CString strName)
{
	try
	{
		...
		GetPrivateProfileString("param","m_nThr","50",tmpchar,MAX_PATH,strName);
		m_nThr=atoi(tmpchar);
		GetPrivateProfileString("param","m_dSigma","30.0",tmpchar,MAX_PATH,strName);
		m_dSigma=atof(tmpchar);

		GetPrivateProfileString("param","m_dWidth","20.0",tmpchar,MAX_PATH,strName);
		m_dWidth=atof(tmpchar);
		GetPrivateProfileString("param","m_dHeight","500.0",tmpchar,MAX_PATH,strName);
		m_dHeight=atof(tmpchar);

		GetPrivateProfileString("param","m_nMeasureNum","20",tmpchar,MAX_PATH,strName);
		...
		GetPrivateProfileString("param","m_nSortType","0",tmpchar,MAX_PATH,strName);
		m_nSortType=atoi(tmpchar);

		/*GetPrivateProfileString("param","m_nPosition","0",tmpchar,MAX_PATH,strName);
		m_nPosition=atoi(tmpchar);*/
		return 0;
	}
	catch (...)
	{
...
int CLineToolPara::WriteParam(CString strName)
{
	try
	{
		...
		tmpstr.Format("%d",m_nSortType);
		WritePrivateProfileString("param","m_nSortType",tmpstr,strName);
	
		/*tmpstr.Format("%d",m_nPosition);
		WritePrivateProfileString("param","m_nPosition",tmpstr,strName);*/
		return 0;
	}
	catch (...)
	{
