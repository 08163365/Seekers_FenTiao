...
BOOL CDlgCalibrate::OnInitDialog()
{
	...
	HTuple Width,Height;
	m_hoImage=m_DlgParamSet->m_hoPretreatImage;
...
void CDlgCalibrate::InitList()
{
	...
	m_hvXPose=m_DlgParamSet->m_pTempDetect->m_hvXPose;
	m_hvColPose=m_DlgParamSet->m_pTempDetect->m_hvColPose;
	m_nPoseCount=m_DlgParamSet->m_pTempDetect->m_nPoseCount;
	m_EDT_dCalSpaceDist=m_DlgParamSet->m_pTempDetect->m_dCalSpaceDist;
	m_EDT_dCoefficient=m_DlgParamSet->m_pTempDetect->m_dCorCoef;
	m_hvRes=m_DlgParamSet->m_pTempDetect->m_hvRes;
	UpdateData(FALSE);
}
void CDlgCalibrate::WriteParam()
{
	UpdateData(TRUE);
	m_DlgParamSet->m_pTempDetect->m_dCalSpaceDist=m_EDT_dCalSpaceDist;
	m_DlgParamSet->m_pTempDetect->m_hvColPose=m_hvColPose;
	m_DlgParamSet->m_pTempDetect->m_hvXPose=m_hvXPose;
	m_DlgParamSet->m_pTempDetect->m_nPoseCount=m_nPoseCount;
	m_DlgParamSet->m_pTempDetect->m_dCorCoef=m_EDT_dCoefficient;
	m_DlgParamSet->m_pTempDetect->m_hvRes=m_hvRes;
	CDialogEx::OnCancel();
}
